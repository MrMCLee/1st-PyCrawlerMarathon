...
class JSONPipeline(object):
    def open_spider(self, spider):
        ...
        print('-'*100)
        print(
            f'\t >>>>>>>>>>>>>>>>>>>>> runtime_file_path :{self.runtime_file_path}')
        print('-'*100)
    ...
    def close_spider(self, spider):
        ...
        if spider.name == 'PTTCrawler' and spider.board:
            self.store_file_path = self.dir_path / '{board}-{datetime}.json'.format(board=spider.board, datetime=datetime.now().strftime('%Y%m%dT%H%M%S'))
        
        self.store_file_path = str(self.store_file_path)
        print('='*100)
        print(
            f'\n\nstore_file_path :{self.store_file_path}\n\nruntime_file_path = {self.runtime_file_path}\n\n')
        print('='*100)
